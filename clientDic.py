...
pygame.display.set_caption("Admiral Acknowledgement")
...
class game_grid():
    ...
    def __init__(self, x, y):
        ...
        self.rect = pygame.draw.rect(window, self.color, [self.x, self.y , square_length, square_width])
        self.island = False
...
class Player():
    ...
    def move(self):
        # Add movement validation and slow down
        ...
        if keys[pygame.K_LEFT]:
            if (playerX - 100 >= 50) and game_dictionary[chr(ord(KEY) - 1)][COUNT].island == False:
                self.x -= square_width
                KEY = chr(ord(KEY) - 1)
                playerX -= 100
                WAITING = True
        if keys[pygame.K_RIGHT]:
            if (playerX + 100 <= 950) and game_dictionary[chr(ord(KEY) + 1)][COUNT].island == False:
                self.x += square_width
                KEY = chr(ord(KEY) + 1)
                playerX += 100
                WAITING = True
        if keys[pygame.K_UP]:
            if (playerY - 100 >= 50) and game_dictionary[KEY][COUNT - 1].island == False:
                self.y -= square_length
                COUNT -= 1
                playerY -= 100
                WAITING = True
        if keys[pygame.K_DOWN]:
            if (playerY + 100 <= 950) and game_dictionary[KEY][COUNT + 1].island == False:
                self.y += square_length
                COUNT += 1
                playerY += 100
                WAITING = True
        self.rect = (self.x + 25, self.y + 25, self.width, self.height)
...
def build_game_grid():
    ...
    for key in game_dictionary.keys():
        for count, index in enumerate(game_dictionary[key]):
            game_dictionary[key][count].key = key
            game_dictionary[key][count].count = count
...
def print_game_grid():
    ...
    for x in range(grid_width):
        pygame.draw.rect(window, black, (x * square_width + 50, 0, 2, win_height))
    for y in range(grid_length):
        pygame.draw.rect(window, black, (0, y * square_length + 50, win_height, 2))
